...
import type { MockTrainingStatus, TrainingReadiness } from '../../types';
import { createTrainingJob, exportTrainingDataset, getTrainingReadiness, listTrainingJobs, returnToBaseModel, startTrainingJob, useModelVersion } from '../../api/training';

interface ObjectTrainingPanelProps { readiness: TrainingReadiness; }
...
export const ObjectTrainingPanel: React.FC<ObjectTrainingPanelProps> = ({ readiness }) => {
  const [status, setStatus] = useState<MockTrainingStatus>('idle');
  const [activateConfirmOpen, setActivateConfirmOpen] = useState(false);
  const [rollbackConfirmOpen, setRollbackConfirmOpen] = useState(false);
  const [remoteReadiness, setRemoteReadiness] = useState<TrainingReadiness | null>(null);
  ...
  React.useEffect(() => {
    ...
    const refresh = () => getTrainingReadiness().then((result) => {
      if (active) setRemoteReadiness({ savedSamples: result.savedSamples, labelsWithSamples: result.labelsWithSamples, sourceCount: result.sourceCount, excludedSamples: result.excludedSamples, isReady: result.ready, issues: result.issues, labelCoverage: result.labelCoverage });
    }).catch(() => undefined);
    refresh();
    const timer = window.setInterval(refresh, 8_000);
    return () => { active = false; window.clearInterval(timer); };
  }, [readiness.savedSamples]);
  ...
  const effectiveReadiness = useMemo<TrainingReadiness>(() => remoteReadiness || readiness, [readiness, remoteReadiness]);
  ...
  const canStart = effectiveReadiness.isReady && (status === 'idle' || status === 'failed');
  ...
  return (
    <section className="glass-panel" aria-label="Cải thiện nhận diện" style={{ borderRadius: '16px', padding: '15px', border: '1px solid var(--line)', boxShadow: 'var(--shadow-md)' }}>
      ...
      {effectiveReadiness.labelCoverage?.length ? <div style={{ background: 'var(--raise)', border: '1px solid var(--line)', borderRadius: '10px', padding: '9px 10px', fontSize: '10.5px', lineHeight: 1.45, marginBottom: '10px' }}>
        <strong style={{ color: 'var(--ink)' }}>Bộ train bãi kiểm: Container · Xe tải · Xe nâng</strong>
        {effectiveReadiness.labelCoverage.map((item) => <div key={item.label} style={{ color: item.ready ? 'var(--ok)' : 'var(--ink2)', marginTop: '4px', fontFamily: 'var(--font-mono)' }}>{item.label}: {item.savedSamples}/{item.minimumSamples} ô, {item.sourceCount}/{item.minimumSources} nguồn, train/val/test {item.splitCounts.train}/{item.splitCounts.val}/{item.splitCounts.test}</div>)}
        <div style={{ color: 'var(--ink3)', marginTop: '6px' }}>Trên mỗi khung hình dùng để train, hãy vẽ tất cả Container/Xe tải/Xe nâng nhìn thấy; nhãn khác vẫn giữ nguyên để chọn cho Zone.</div>
      </div> : null}

...
|---|---|---|---|---|---|---|---|---|
| FDN-REPO-SCAFFOLD | Repository scaffold: python-worker, node-api, .env.example, scripts | None — only `frontend/` and `docs/` exist | `python-worker/`, `node-api/` directories with package manifests, `.env.example`, `data/clips/`, `data/crops/` | All slices | M | Hữu Thuận | planned | directories exist, `pip install` + `npm install` succeed |
| FDN-DB-MIGRATION | Prisma schema + migration cho 7 tables | None — greenfield | `node-api/prisma/schema.prisma` with all tables, `npx prisma migrate deploy` succeeds against Neon | All slices except VS-QA-CHAT (indirectly) | M | Hữu Thuận | planned | `npx prisma migrate deploy` exits 0, tables exist in Neon |
| FDN-PYTHON-DB | Python asyncpg client + connection pool for Neon | None | `python-worker/db/` module with connection pool, basic CRUD helpers | VS-GATE-LIVE, VS-AREA-VIOLATION | S | Hữu Thuận | planned | Python can connect and SELECT from Neon |
| FDN-WS-PROXY | Node.js WebSocket proxy: Python→Node→Browser | None | `node-api/src/ws/` WebSocket proxy accepting Python frames + events, forwarding to browser clients | VS-GATE-LIVE, VS-AREA-VIOLATION | M | Hữu Thuận | planned | Browser receives test frame via WS chain |
| FDN-PYTHON-STREAM | Python AI Worker: OpenCV stream reader + YOLO inference pipeline | None | `python-worker/stream/` + `python-worker/detection/` with frame read → YOLO detect → WS emit | VS-GATE-LIVE, VS-AREA-VIOLATION | M | Hữu Thuận | planned | Python reads video file, runs YOLO, emits annotated frames via WS |
| FDN-API-CONTRACT | Node.js Express REST scaffold + API versioning + error contract | None | `node-api/src/routes/` with health check, error middleware, CORS localhost | All backend slices | S | Hữu Thuận | planned | `GET /api/v1/health` returns 200 |
| FDN-FRONTEND-API | Frontend API client + WebSocket hooks | None — currently all mock data | `frontend/src/api/` + `frontend/src/hooks/useWebSocket.ts`, `useBroadcastChannel.ts` | All frontend slices | S | Hữu Thuận | planned | Frontend can fetch from Node.js API and receive WS messages |
